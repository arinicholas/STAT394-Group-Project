...
- The first two principal components captured most of the variation in the data\linebreak
- These two components were dominated by two underlying relationships\linebreak
- We hypothesized that "Overall Diamond Quality" might be able to be explained by these two factors\linebreak
- "Price + Dimension" + "Light Conductance" -> "Overall Diamond Quality"\linebreak
- We tested this hypothesis with a factor analysis\linebreak 

# Factor Analysis

```{r, echo = FALSE}
options(width = 300, length = 200)
a <- factanal(diamonds[,-c(2:4,11)], factors =2)
a$loadings
```
