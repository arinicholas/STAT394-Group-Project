...
```{r normalQQplots, fig.cap= "Normal QQ Plots of Numeric Variables", fig.pos= "center", fig.pos= "H"}
par(mfrow=c(3,3))  
...
```{r densityplots, fig.cap="Density plots for cut, color and clarity by price", echo=FALSE}
...
Figure \@ref(fig:densityplots) shows the density plots for the three categorical variables versus 'price'. 
...
Figure \@ref(fig:denclar2) shows the densities of the different levels of 'clarity' for price. A prominent peak is visible near the left for the better quality levels (IF, best, pink; WS1 second best, purple; WS2, third best, blue). We conducted a Analysis of Variance to see if these differences in mean price across levels of clarity were significant. While we see potential evidence that the ANOVA assumptions of normality and equal variance may be violated, ANOVA is reasonably robust to these violations if the sample size is big enough.  We also conducted a Levene's test to verfiy if the assumption of homogeneity of variance was violated. If significant we then implemented a non-parametric Kruskal Wallis Test. The results are summarised on the table below and the code for theses tests may be found in the appendices.
...
Table \@ref(tab:claranotab) shows the table of results from the three tests on the variable 'clarity'. The output frpm the ANOVA returns a p-value of < 2.2e-16, meaning there is strong evidence to suggest that mean price differs across levels of "clarity". The output of the Levene's test and Kruskal Wallis tests show that there is not equal variances between the levels, and that there is a significant difference between different levels of clarity. A Tukey test showed that most pairwise combinations show a significant difference. There are only six that do not show a difference and they are: SI1-I1; VS2-I1; VS1-I1; VS2-SI1; VS1-VS2; and IF-VVS1.
...
```{r diagplots, fig.cap="Diagnostic plots to check regression assumptions"}
par(mfrow=c(2,2))
plot(best.lm, 1)
plot(best.lm, 2)
plot(best.lm, 3)
...
```{r, eigenvectorplotpc}
Rotation.df <- data.frame(c(pca.diamonds$rotation))
...
```{r biplotpc, fig.cap="Biplot of Principal Components"}
...
```{r pcascreered, fig.cap="Scree plot of the Principal Components (reduced dataset)"}
...
```{r biplotpcared, fig.cap="Biplot of the reduced dataset PC1 vs PC2"}
...
In the above analysis we have shown we are able to us principal components analysis to reduce the number of dimensions in the dataset while still retaining a large proportion of its variation. In this investigation we are interested in predicting the price of diamonds. To do this we will perform a regression with principal components with price as the dependent variable and the other numerical variables as explanatory variables. 
...
```{r pcascreereg, fig.cap="Scree plot of the regression Principal Components"}
...
```{r ordplot, fig.cap="Ordination plots"}
...
LDAacc <- data.frame(Class=c("Cut","Color","Clarity"),
                      Accuracy=c(0.6382, 0.196, 0.1457))
...
The aims of the project were to find the best predictor variable or variables for 'price'. This was achieved using the multiple regression model with all variables except for 'y' and 'z', in other words the model; price = carat + cut + color + clarity + depth + table + x. The best model was obtained using stepwise AIC and BIC tests. The diagnostics for the multiple regression model showed that all the assumptions were violated, meaning the model might not produce accurate predictions. 

A number of erroneous values were uncovered in the dataset, including impossible zero values for various diamond dimensions (height, length, width etc.), and a number of extreme upper outliers that could not reasonably be valid values.

We applied a number of the new analytical methods that we learned in STAT394. As this project was principally a learning task we attempted them all, even though an experienced statistician may have concluded beforehand that they were inappropriate or of dubious value. We mostly performed these techniques on a subset of the data that only included the numerical variables. Having experimented with all of the techniques (listed below), and realising that they were not working as we had hoped, we investigated why. The reason appeared to be the exclusion of the categorical variables, the different levels of which explain so much of the variation in price.  

A principal components analysis was conducted, but this did not perform better than the best multiple linear regression model, mainly because the categorical variables could not be directly included. Factor analysis was performed using one combination of 'dimension' variables and a second combination of 'light performance' variables. This was not successful and the reason was very likely the exclusion of the categorical variables. Linear discriminant analysis was undertaken, but this failed to separate the different levels of the categorical variables due to the amount of overlap between them all. A cluster analysis was conducted; this also resulted in limited success due to the amount of overlap between the different levels of the categorical variables.  
