...
Cut
- A diamonds Cut defines its proportions and its ability to reflect light. This variable has five levels: Fair, Good, Very Good, Premium and Ideal. 
Color
...
We see that in the first PC carat, x, y and z all have approximately equal weighting. Again this reflects the strong positive correlation between these two varaibles. In the second PC depth and table have the strongest weighting. 
...
## Diamonds2: Create the Principal Components object
...
pca.diamonds2 <- prcomp(diamonds2,
                       center=TRUE, scale. = TRUE, retx=TRUE)
plot(pca.diamonds2)
summary(pca.diamonds2)
```

```{r}
pca.diamonds2$rotation
```

```{r}
ggbiplot(pca.diamonds2, obs.scale =1, var.scale = 1)
```
...
```{r pconevpctwo2, fig.cap="Scatterplot of PC1 vs PC2 (encoded version)"}
...
Figure \@ref(fig:pconevpctwo2) shows the scatterplot of PC1 versus PC2. 
...
```{r pcascree2, fig.cap="Scree plot of the Principal Components"}
...
Figure \@ref(fig:pcascree2) shows the scree plot for the Principal Components of the diamonds dataset. PC1 contains `r pca.var.per2[1]`% of the variance, PC2 contains `r pca.var.per2[2]`%, while PC3 contains `r pca.var.per2[3]`%. Between them they contain `r sum(pca.var.per2[1:3])`%, which is clearly very good.   
...
# Multiple regression using diamonds2 (numerically coded)
...
### Modified version of dataset only containing carat values between 1 and 1.1
