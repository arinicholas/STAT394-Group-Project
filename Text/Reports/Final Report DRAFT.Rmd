...
Our primary aim was to identify which variables best predictor the price of any given diamond. We hypothesise that increased diamond size (x, y and z) and weight will be positively correlated with diamond price. We also expect to see increased prices for diamonds with higher levels of the categorical variables. We are unsure how diamond depth and table percentage while relate to diamond price. 

We also seek to explore the techniques of Principal Component Analysis (PCA), Linear Discriminant Analysis (LDA) and Cluster Analysis (CA) on the diamonds dataset. The diamonds dataset has 280 unique possible interactions between different levels of the categorical variables. We will attempt to use the techniques of LDA and CA to more simply classify the diamonds data.

# Methodology

The diamonds data set was accessed via the kaggle.com website while all statistical analysis and reporting was completed in R/R Markdown [@R-base]. Github [@github] was used a repository for storing all relevant documents and code during this project.
...
To begin our investigation we undertook an in-depth Exploratory Data Analysis (EDA) of the data. The key results from the EDA can be found in the results section below. We used this to develop our leading questions that we attempt to answer using the multivariate statistical techniques of Principal Components Analysis, Factor Analysis, Linear Discriminant Analysis and Cluster Analysis. 
