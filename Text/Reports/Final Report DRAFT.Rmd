...
Table \@ref(tab:colortab) shows the number of diamonds in each level of the 'color' variable. As a reminder, 'J' is the lowest level and 'D' is the highest. Note that the highest values are in the middle of the range of levels.
...
The aims of the project were to find the best predictor variable or variables for 'price'. This was achieved using the multiple regression model with all variables except for 'y' and 'z', in other words the model; price = carat + cut + color + clarity + depth + table + x. The diagnostics for the multiple regression model showed that all the assumptions were violated, meaning the model might not produce accurate predictions.   
