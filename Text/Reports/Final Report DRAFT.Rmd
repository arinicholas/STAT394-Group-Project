...
We also seek to explore the techniques of Linear Discriminant Analysis and Cluster Analysis on the diamonds dataset. The diamonds dataset has 280 unique possible interactions between different levels of the categorical variables. We will attempt to use the techniques of LDA and CA to more simply classify the diamonds data.
...
LDAcut <- lda(cut ~ carat + depth + table + price + x + y + z, data = diamonds.Train)
LDAcolor <- lda(color ~ carat + depth + table + price + x + y + z, data = diamonds.Train)
LDAclarity <- lda(clarity ~ carat + depth + table + price + x + y + z, data = diamonds.Train)
```
...
All three plots show a lot of overlap between different levels of the categorical variables. It seems unlikely that a linear discriminant function would have much success in partition the data according to these classes. We shall compute the confusion matrix for each LDA and assess the overall accuracy of each model.

```{r}
RPcut <- predict(LDA, diamonds.Test)$class
RCMcut <- table(RPcut, actual = diamonds.Test$cut)
RPcolor <- predict(LDA, diamonds.Test)$class
RCMcolor <- table(RPcut, actual = diamonds.Test$color)
RPclarity <- predict(LDA, diamonds.Test)$class
...
From using the elbow method we see that 4 clusters is the optimum number.
...
In the cluster plot we see significant overlap between the four clusters as well as many observations that do not seem to fit well in any of the clusters. It seems that the diamond dataset does not easily split into distinct clusters.
