...
Looking at the eigenvectors we see that the variables of carat, price, x, y and z are all have equal weight in PC1. The variables with the most weight in PC2 are depth and table. We see a similar patten in PC3 but now price is also has more weight. This makes sense as in our examination of correlations between variables we saw that price, x, y, z and carat were all strongly correlated. The opposite loadings of depth and table in PC2 and PC3 reflect there negative correlation.  A visualization of the PCA is shown in the Biplot below. 
...
Due to the vast number of data entries it is hard to interpret what is going on from the visual display. We will not take a smaller sample of 300 from the diamonds data and redo the PCA after first checking that the smaller sample has similar properties to the full sample.

```{r}
set.seed(300525287)
smallersample <- diamonds[sample(nrow(diamonds), 300), ]
...
We see that a PCA on the smaller sample results in close to identical results. The first PC now accounts  for 69% of the variance, the second PC accounts for 17% and the third PC accounts for 12%. Together the first three components account for 97% of the variance in the dataset.

```{r}
ggbiplot(reducedpca, obs.scale =1, var.scale = 1, groups = smallersample$clarity)
```
...
We see from this output that all but one predictor variable (y) are significant in this model when the effects of the other predictors are already accounted for. This model explain 88% of the variation in price. We will now check the regression assumptions.
...
The plot above shows the proportion of variation explained by each of the principal component. The first three principal components explain nearly all (99%) the variation in the data. 
...
With price not included it becomes much harder to distinguish which variables have the strongest influence in each principal component. Multiple variables have influence in each of the first three principal components. 
