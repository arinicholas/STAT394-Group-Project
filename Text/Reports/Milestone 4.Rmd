...
We have strong evidence to suggest that mean price differs across levels of "cut". We will no perform a Tukey Test to determine which pairwise differences are significant
...
summary(cutanova)
```
We have strong evidence to suggest that mean price differs across levels of "cut". We will no perform a Tukey Test to determine which pairwise comparisons are significant

```{r}
TukeyHSD(coloranova, conf.level = 0.95)
```

We see significant differences in mean price for nearly all pairwise comparisons of diamond colours.
