...
Table \@ref(tab:summtablediam) presents the summary statistics for the numerical variables in the diamonds dataset. We see that the kurtosis of every single numerical variable is much larger than we would expect if each variable was normally distributed. A postive skewness indicates that most of the variables are skewed to the right. We see that carat, price, y and z are highly skewed to the right.
...
```{r}
ad.test(diamonds_num$depth)
```

Again the QQplot for depth (figure \@ref(fig:qqcaratlt)) shows substantial deviations from the Normal distribution at the tails. The Anderson-Darling test returned a p-value of < 2.2e-16 (machine precision zero). Therefore, we reject the null hypothesis that the data is normally distributed. 
...
```{r}
ad.test(diamonds_num$table)
```

Figure \@ref(fig:qqtable) of the 'table' variable also suggests deviations from Normality with the upper tail in particular sweeping upward away from the predicted line. The Anderson-Darling test returned a p-value of < 2.2e-16, confirming that the data does not follow a Normal distribution. 
