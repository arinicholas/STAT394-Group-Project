...
Our primary aim was to identify which variables best predict the *price* of a given diamond. We hypothesised that increased diamond size ('x', 'y' and 'z') and weight ('carat') would be positively correlated with diamond price. We also expected to see increased prices for diamonds with higher levels of the categorical variables, all of which reflect the quality of the diamond. We were unsure how diamond depth and table percentage would relate to diamond price. 
...
\hfill\break
We decided to investigate outliers and unusual points in our dataset. These points may have increased leverage or influence when we come to using variables to predict price. We proceeded with these tests despite knowing the data was not Normally distributed as the Mahalanobis Distance was new to us all and it was a useful learning exercise.   
...
#### Zero Values 
\hfill\break
In this section we identify any zero values and determine whether or not they are errors. This was something that came to the attention of the team after the initial EDA had been submitted, hence the reason for it's inclusion here. The code below shows the lowest values for each of the seven numerical variables. The number of elements to display was tweaked until the output showed all the zero values (if any). This was partially to discover whether the extreme values in the dataset were having undue influence on results. 
...
We now display the zero values for each of the variables, noting whether the other variables are consistent with zero readings, or whether they indicate that this is impossible. For example, the diamond in row 1 below with zero mm length ('x'), but 6.62 mm width ('y').
...
Similarly with the extreme value of 'z' (31.80). The other dimensions of this diamond do not tally with the extreme depth value, nor does the relatively low price. The next highest value has been included for comparison (8.06). So this outlier is almost certainly an error.
...
As we have 3 categorical variables: color, clarity and cut with 7, 8 and 5 levels respectively. This means there are 280 separate interactions in our data set.
...
A visualization of the eigenvectors of the first two principal components is shown in figure \@ref(fig:eigenvectorplotpc).
...
```{r colourcodedbiplots, fig.cap="Colour Coded Biplot" , echo = FALSE}
a1 <- ggbiplot(reducedpca, obs.scale =1, var.scale = 1, varname.size = 5, groups = smallersample$cut)
b1 <- ggbiplot(reducedpca, obs.scale =1, var.scale = 1, varname.size = 5, groups = smallersample$color)
c1 <- ggbiplot(reducedpca, obs.scale =1, var.scale = 1, varname.size = 5, groups = smallersample$clarity)
ggarrange(a1, b1, c1, ncol = 2, nrow =2)
```

We also thought it would be interesting to examine the biplots with observations projected onto the first two principal components coded for different levels of our categorical variables and to examine if any patterns emerge. Unfortunately in figure \@ref(fig:colourcodedbiplots) we don't see any clear clustering or pattern emerge with through the categorical variables. This indicates it may be hard to classify the data using the categorical variables. 
...
```{r pcbiplot, fig.cap="Biplot of the PCs", echo=FALSE}
ggbiplot(PCAprice, obs.scale =1, var.scale = 1, varname.size = 5)
```

Even with price not included we still see the same general structure as the PCA with price (figure \@ref(fig:pcbiplot)).
...
## LDA Predicting Price 

We attempted a number of versions of LDA: one with price split into three categories; one with the full dataset; and one with a reduced slice. 
### LDA - Price split into three categories

The secondary aim of our project was to investigate methods of classifying the diamonds dataset more simply. We decided to investigate whether, if we split diamond price into three subgroups (low, medium and high) we could accurately predict which of these groups a diamond should belong too. We split the diamonds into the three subgroups using the upper and lower quartiles (UQ and LQ). Diamonds were classed as having "low" price if they were in the LQ and classed as "high" if they were in the UQ. Diamonds were classed as having "medium" price if they were in between. We then used an LDA with the other numeric variables to see how well we could classify diamonds into there price levels. 

```{r, echo = FALSE}
diamonds$pricerank <- diamonds %>% mutate(pricerank = case_when(price <= 950 ~ 'low',
                                                   price >= 950 & price <= 5324.5 ~ 'med',
                                                   price >= 5324.5 ~ 'high'))
...
(LDApricerank <- lda(pricerank$pricerank ~ carat + depth + table + x + y + z, data = diamonds.Train))
```

The first thing we notice from the LDA output is the the prior probabilities are not equal but match up very closely with the expected probabilities given we took the upper and lower quartiles to rank diamond price. Examining the group means tells us that depth and table do not vary much between levels of diamond price whereas the other variables do. We also see that the first linear discriminant function retains a large proportion of the variability. We can visualize the LDA in the graph below.

```{r LDApricerank, fig.cap="Ordination Plot for LDA of Price ", echo=FALSE}
ggord(LDApricerank, diamonds.Train$pricerank$pricerank, alpha= .5)
```

The ordination plot (figure \@ref(fig:LDApricerank)) shows some evidence of the three groups being seperable with a linear function. There is still some overlap as well as some points (or groups of points) that seem to be far removed from the rest of the data. We will now see how this LDA performs as a classifier. 


```{r, echo = FALSE}
RealisticPredictedPR <- predict(LDApricerank, diamonds.Test)$class
RCMPR <- table(RealisticPredictedPR, Actual = diamonds.Test$pricerank$pricerank)
confusionMatrix(RCMPR)
```

We see that LDA was reasonably successful at classifying diamonds into the three price levels of low, medium and high with an overall accuracy of 84%. As the prior probabilities of classes were not equal we should also check the within-class accuracy. We see that all classes performed well with "low" having the highest balanced accuracy. We can conclude that on our modified price variable, we are able to predict the value of a diamond reasonably accurately with an LDA.  

## LDA predicting categorical variables 


As discussed previously the diamonds data set has three categorical variables each with many levels. This means we have the possibility of 280 unique interactions between the levels of these categorical variables. If we could easily classify a diamond into the correct factor level based on the numeric variables this would be ideal however it is likely to be difficult to classify diamonds using this many factors.  We will begin by verifying whether predicting the factor level (out of 280) that a diamond will belong too is as complex a task as we expect by using an LDA.  
...
ind <- sample(c("Train", "Test"),
              ...
              replace = TRUE, 
              prob = c(.8, .2))
diamonds.Train2 <- diamonds[ind == "Train",]
diamonds.Test2 <- diamonds[ind == "Test",]
LDAinteraction <- lda(Interaction ~ carat + depth + table + price + x + y + z, data = diamonds.Train2)
```
...
```{r, echo= FALSE}
RPinteraction <- predict(LDAinteraction, diamonds.Test2)$class
RCMinteraction <- table(RPinteraction, actual = diamonds.Test2$Interaction)
sum(diag(RCMinteraction))/sum(RCMinteraction)
```

An overall accuracy of 0.065 is very low and therefore we can conclude that an LDA with all other variables does a poor job at predicting which factor level a diamond should be classified as. It's likely the data is not linearly separable into 280 classes.   

We will now see if a Linear Discriminant Analysis using the numerical variables can be used to accurately predict what level of a categorical variable a diamond belongs to. We will attempt this for each categorical variable. If successful, we will have found a simple way to classify diamonds based on the numeric variables in the data set. This could be useful as the categorical variables are likely to be the most time consuming to assess, likely needing expert evaluation. If we can accurately predict what level of a categorical variable a diamond will belong to based on the other, more easily measured, variables this could have real-world, practical benefits. 
 

```{r}
LDAcut <- lda(cut ~ carat + depth + table + price
              + x + y + z, data = diamonds.Train2)
...
g1 <- ggord(LDAcut, diamonds.Train2$cut, alpha = 0.5, )
g2 <- ggord(LDAcolor, diamonds.Train2$color, alpha = 0.5 )
g3 <- ggord(LDAclarity, diamonds.Train2$clarity, alpha = 0.5)
...
```{r, echo=FALSE}
RPcut <- predict(LDAcut, diamonds.Test2)$class
RCMcut <- table(RPcut, actual = diamonds.Test2$cut)
RPcolor <- predict(LDAcolor, diamonds.Test2)$class
RCMcolor <- table(RPcut, actual = diamonds.Test2$color)
RPclarity <- predict(LDAclarity, diamonds.Test2)$class
RCMclarity <- table(RPcut, actual = diamonds.Test2$clarity)
```
...
We conclude that LDA is not particularly effective at correctly classifying diamonds into different levels of the categorical variables. Using the levels of the categorical variables as classes was more successful than using the interactions with the cut variable returning the highest accuracy. Even this was not particularly successful. We therefore conclude the data is not linearly separable by using the levels of categorical variables as classes.  
...
So, using 'carat' alone to predict these prices clearly is not enough; given the spread of price for any given value of carat, the categorical variables play an important role in determining where in the range any given diamond will be. This was the reason our regression models that excluded the categorical variables did not perform nearly as well as those that included them.


## Discriminant analysis using the reduced dataset (price, carat and clarity)

To address the issue above, we attempted another discriminant analysis using the reduced dataset above, that is, for 'carat' values between 1.0 and 1.1. The hope was that the relatively clear bands seen in figure \@ref(fig:carat1to11clarity) would enable the LDA to work this time, as there is less overlap between levels in the reduced dataset than there is in the original.
...
While LDA returned some positive results in terms of classification were were interested to investigate how a Cluster Analysis would perform with the diamonds data. We used a K-means algorithm to split the data into distinct clusters. Therefore centroids are computed as the mean of all observations belonging to each class. The Euclidean Distance was used for intra/inter class distances. As we were initially unsure of how many clusters we should attempt to divide the data into we will first examine the sum of the squares of the intra-class disatances for different numbers of clusters. We will again use the smaller sample to aid in the interpretation of visual displays. 
...
The silhouette plot (figure \@ref(fig:silh)) helps us to visualize how similar each observation is to the cluster that it is assigned too. We see very few negative values indicating that very few observations have been assigned to the wrong cluster. However we do have a large proportion of values with low positive values indicating that these observations were close to the borderline between two clusters. We see that the average silhouette width is 0.38. 
